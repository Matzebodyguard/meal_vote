...
Dashboard-Ressource: `/meal_vote_static/meal-vote-card.js?v=0.6.22`
...
### Wochenplan (v0.6.22)
Zusätzliche Ressource: `/meal_vote_static/meal-week-plan-card.js?v=0.6.22`
...
grid_options:
  columns: full
```
Ressource: `/meal_vote_static/meal-week-plan-card.js?v=0.6.22`

Verwaltung:
```yaml
type: custom:meal-week-plan-admin-card
grid_options:
  columns: full
```
Ressource: `/meal_vote_static/meal-week-plan-admin-card.js?v=0.6.22`

Nur die Admin-Karte enthält Bearbeiten, Löschen, „＋ Gericht“ und den Wocheneinkauf.


### Rezepte und mehrere Kategorien (v0.6.22)
...
```csv
dish_id,recipe
spaghetti,"Nudeln kochen. Sauce zubereiten und vermengen."
```
...
### Bildoptimierung (v0.6.22)

Neue Uploads werden automatisch:
- auf maximal 1200 × 1200 Pixel verkleinert,
- als WebP mit Qualitätsstufe 82 gespeichert,
- für die Übersicht zusätzlich als Thumbnail mit maximal 480 × 480 Pixel gecacht.
...
### Copy-&-Paste-Rezeptimport (v0.6.22)
...
### Icon
Ab v0.6.22 liegt ein quadratisches Installations-/Repository-Icon als `icon.png` im Repository sowie im Integrationsordner bei. Für die offizielle Anzeige des Icons in Home Assistant/HACS kann das Icon zusätzlich im Home-Assistant-Brands-Repository für die Domain `meal_vote` hinterlegt werden.


### Lokaler Speichermodus (v0.6.22)
...
### Automatische Speichermigration (v0.6.22)

Wird in den Integrationsoptionen zwischen **Lokal auf Home Assistant** und **Netzwerk / NAS** gewechselt, kopiert Meal Vote die CSV-Daten und den `images/`-Ordner automatisch zum neuen Speicherort. Die Quelldaten bleiben als Sicherheitskopie erhalten. Bereits vorhandene CSV-Dateien am Ziel werden vor dem Überschreiben als `.before_migration` gesichert.


### Lokales Home-Assistant-Branding (v0.6.22)

Ab Home Assistant 2026.3 kann die Integration ihr Branding direkt mitliefern:

```text
custom_components/meal_vote/brand/
├── icon.png       # 256 × 256
└── icon@2x.png    # 512 × 512
```

Home Assistant verwendet diese lokalen Branding-Dateien automatisch für die Domain `meal_vote`.
