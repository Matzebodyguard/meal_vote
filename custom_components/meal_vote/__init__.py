...
from .const import DEFAULT_PEOPLE, DOMAIN, SYNC_MINUTES
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    for command in (ws_get_data, ws_add_dish, ws_update_dish, ws_delete_dish, ws_upload_image):
        websocket_api.async_register_command(hass, command)
    return True


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    manager = MealVoteManager(hass, entry.data["data_path"], _people(entry))
...
@websocket_api.websocket_command({vol.Required("type"): "meal_vote/add_dish", vol.Required("name"): str, vol.Optional("category", default=""): str, vol.Optional("image", default=""): str})
@websocket_api.async_response
async def ws_add_dish(hass, connection, msg):
    try:
        dish_id = await _manager(hass).async_add_dish(msg["name"], msg.get("category", ""), msg.get("image", ""))
        connection.send_result(msg["id"], {"dish_id": dish_id})
    except (ValueError, OSError) as err:
        connection.send_error(msg["id"], "save_failed", str(err))


@websocket_api.websocket_command({vol.Required("type"): "meal_vote/update_dish", vol.Required("dish_id"): str, vol.Required("name"): str, vol.Optional("category", default=""): str, vol.Optional("image", default=""): str, vol.Optional("active", default=True): bool})
@websocket_api.async_response
async def ws_update_dish(hass, connection, msg):
    try:
        await _manager(hass).async_update_dish(msg["dish_id"], msg["name"], msg.get("category", ""), msg.get("image", ""), msg.get("active", True))
