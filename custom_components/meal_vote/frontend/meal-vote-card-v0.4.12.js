class MealVoteCard extends HTMLElement {
  ...
  ingredientMatches(value){
    const q = String(value || '').trim().toLocaleLowerCase('de-DE');
    const catalog = this.ingredientCatalog();
    if (!q) return catalog.slice(0,8);
    return catalog
      .filter(item => item.name.toLocaleLowerCase('de-DE').includes(q))
      .slice(0,8);
  }
...
if(!customElements.get('meal-vote-card-v0412')) customElements.define('meal-vote-card-v0412',MealVoteCard);console.info('[meal_vote] UI 0.4.12 loaded');window.customCards=window.customCards||[];window.customCards.push({type:'meal-vote-card-v0412',name:'Essenswahl',description:'Familien-Voting für Gerichte'});
