class MealWeekPlanAdminCard extends HTMLElement{
  constructor(){super();this.attachShadow({mode:'open'});this.hass=null;this.data={dishes:[],pantry:[],week_plan:{},week_plan_next:{}};this.weekView='current';}
  ...
  dish(id){
    if(id==='__away__')return{id:'__away__',name:'Wir sind nicht da!',special:true,image_url:'/meal_vote_static/special-away.webp?v=0.6.27'};
    if(id==='__bread__')return{id:'__bread__',name:'Brot',special:true,image_url:'/meal_vote_static/special-bread.webp?v=0.6.27'};
    return this.data.dishes.find(d=>d.id===id);
  }
  specialDishes(){return[{id:'__away__',name:'Wir sind nicht da!',special:true,image_url:'/meal_vote_static/special-away.webp?v=0.6.27'},{id:'__bread__',name:'Brot',special:true,image_url:'/meal_vote_static/special-bread.webp?v=0.6.27'}];}
  activeDishes(){
  ...
  async save(plan){await this.ws('meal_vote/set_week_plan',{plan,week:this.weekView});this.data[this.weekView==='next'?'week_plan_next':'week_plan']=plan;this.render();}
  currentPlan(){return this.weekView==='next'?(this.data.week_plan_next||{}):(this.data.week_plan||{});}
  render(){
    const plan=this.currentPlan(), dishes=this.activeDishes();
    this.shadowRoot.innerHTML=`<style>
      ...
      .weekShell{width:100%;max-width:none;min-width:0}.weekTabs{display:flex;gap:8px;margin:0 0 12px}.weekTabs button{border:1px solid var(--divider-color);background:var(--card-background-color);color:var(--primary-text-color);border-radius:10px;padding:8px 12px;cursor:pointer}.weekTabs button.active{background:var(--primary-color);color:var(--text-primary-color);font-weight:700}
    ...
    </style><ha-card><div class="weekShell"><div class="head"><h2>🛠️ Wochenplan verwalten</h2><span class="badge">UI 0.6.27</span><button id="reload" class="clear">↻</button></div>
      <div class="weekTabs"><button data-week="current" class="${this.weekView==='current'?'active':''}">Diese Woche</button><button data-week="next" class="${this.weekView==='next'?'active':''}">Nächste Woche</button></div>
    ...
    this.shadowRoot.querySelectorAll('[data-week]').forEach(b=>b.onclick=()=>{this.weekView=b.dataset.week;this.render();});
    this.shadowRoot.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{const p=structuredClone(this.currentPlan());(p[b.dataset.add]??=[]).push('');this.data[this.weekView==='next'?'week_plan_next':'week_plan']=p;this.render();});
    ...
    this.shadowRoot.querySelectorAll('[data-remove]').forEach(b=>b.onclick=async()=>{const [day,idx]=b.dataset.remove.split(':');const p=structuredClone(this.currentPlan());p[day]=(p[day]||[]).filter((_,i)=>i!==Number(idx));await this.save(p);});
  ...
  openMealPicker(day,idx){
    ...
    const render=()=>{
      ...
      modal.querySelectorAll('[data-pick]').forEach(b=>b.onclick=async()=>{
        const p=structuredClone(this.currentPlan());
        p[day]=p[day]||[];
  ...
  openShopping(){
    const planned=[];for(const [day] of this.days())for(const id of (this.currentPlan()?.[day]||[])){const d=this.dish(id);if(d)planned.push(d);}
    ...
    modal.innerHTML=`<h2>🛒 ${this.weekView==='next'?'Einkauf nächste Woche':'Wocheneinkauf'}</h2><div>Standardvorrat 🏠 ist nicht vorausgewählt.</div><div class="list">${rows||'Keine Zutaten hinterlegt.'}</div><div class="actions"><button id="none">Keine</button><button id="all">Alle</button><button id="cancel">Abbrechen</button><button id="send">Auswahl hinzufügen</button></div>`;
