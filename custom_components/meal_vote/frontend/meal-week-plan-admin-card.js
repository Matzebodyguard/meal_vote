class MealWeekPlanAdminCard extends HTMLElement{
  ...
  dish(id){
    if(id==='__away__')return{id:'__away__',name:'Wir sind nicht da!',special:true,image_url:'/meal_vote_static/special-away.webp?v=0.6.25'};
    if(id==='__bread__')return{id:'__bread__',name:'Brot',special:true,image_url:'/meal_vote_static/special-bread.webp?v=0.6.25'};
    return this.data.dishes.find(d=>d.id===id);
  }
  specialDishes(){return[{id:'__away__',name:'Wir sind nicht da!',special:true,image_url:'/meal_vote_static/special-away.webp?v=0.6.25'},{id:'__bread__',name:'Brot',special:true,image_url:'/meal_vote_static/special-bread.webp?v=0.6.25'}];}
  activeDishes(){
  ...
  render(){
    ...
    </style><ha-card><div class="weekShell"><div class="head"><h2>🛠️ Wochenplan verwalten</h2><span class="badge">UI 0.6.25</span><button id="reload" class="clear">↻</button></div>
    ...
    this.shadowRoot.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{const p=structuredClone(this.currentPlan());(p[b.dataset.add]??=[]).push('');this.data.week_plan=p;this.render();});
