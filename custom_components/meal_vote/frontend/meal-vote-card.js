class MealVoteCard extends HTMLElement {
  ...
  render(){
    ...
    this.shadowRoot.innerHTML=`<style>
      ...
      <div class="top"><input class="search" id="search" placeholder="🔎 Gericht, Kategorie, Person oder Zutat suchen…" value="${this.esc(this._search)}"><select id="sort"><option value="votes">Meiste Stimmen</option><option value="oldest">Lange nicht gekocht</option><option value="recent">Zuletzt gekocht</option><option value="name">Name</option></select><span style="font-size:.8rem;opacity:.7;font-weight:700">UI 0.4.16</span><button id="reload">↻ Sync</button><button id="pantry">🏠 Standardvorrat</button><button id="inactive">${this._showInactive?'Aktive':'Verwaltung'}</button><button id="add" class="add">＋ Gericht</button></div>
      ...
      <dialog id="dishDialog"><div class="modal" id="dishModal"></div></dialog><dialog id="voteDialog"><div class="modal" id="voteModal"></div></dialog><dialog id="infoDialog"><div class="modal" id="infoModal"></div></dialog><dialog id="shoppingDialog"><div class="modal" id="shoppingModal"></div></dialog><dialog id="pantryDialog"><div class="modal" id="pantryModal"></div></dialog>
    </ha-card>`;
  ...
  openPantryDialog(){
    ...
    modal.innerHTML=`<h2>🏠 Standardvorrat</h2><div class="hint">Diese Zutaten sind im Einkaufsdialog standardmäßig nicht angehakt.</div><input id="pantrySearch" placeholder="🔎 Zutat suchen…" style="width:100%;margin:12px 0"><div id="pantryList" class="ingredientList"></div><div class="modalActions"><button id="pCancel">Abbrechen</button><button id="pSave">Speichern</button></div>`;
  ...
  ingredientEditorHtml(items){const units=['g','kg','ml','l','Stück','Dose','Packung','EL','TL','Prise','Bund'];const names=this.ingredientCatalog();return `<div class="ingredientHead"><span>Zutat</span><span>Menge</span><span>Einheit</span><span></span></div><div class="ingredientEditor" id="ingredientEditor">${items.length?items.map((i,idx)=>`<div class="ingredientRow" data-ing-row="${idx}"><div class="ingredient-name-wrap"><input data-ing-name="${idx}" autocomplete="off" list="ingredientNames" placeholder="z. B. Tomaten" value="${this.esc(i.name||'')}"><div class="ingredient-suggestions"></div></div><input data-ing-amount="${idx}" inputmode="decimal" placeholder="z. B. 500" value="${this.esc(i.amount||'')}"><input data-ing-unit="${idx}" list="ingredientUnits" placeholder="z. B. g" value="${this.esc(i.unit||'')}"><div class="ingredientTools"><button type="button" data-ing-home="${idx}" title="Als Standardvorrat markieren">🏠</button><button type="button" data-ing-up="${idx}" title="Nach oben" ${idx===0?'disabled':''}>↑</button><button type="button" data-ing-down="${idx}" title="Nach unten" ${idx===items.length-1?'disabled':''}>↓</button><button type="button" data-ing-delete="${idx}" class="danger" title="Zutat löschen">✕</button></div></div>`).join(''):'<div class="ingredientEmpty">Noch keine Zutaten. Mit „＋ Zutat“ kannst du die erste anlegen.</div>'}</div><datalist id="ingredientNames">${names.map(x=>`<option value="${this.esc(x.name)}"></option>`).join('')}</datalist><datalist id="ingredientUnits">${units.map(u=>`<option value="${u}"></option>`).join('')}</datalist><button type="button" id="addIngredient" class="ingredientAdd">＋ Zutat</button>`;}
  openDishDialog(dish=null){
    ...
    modal.querySelector('#dSave').onclick=async()=>{syncDraftFromInputs();const name=modal.querySelector('#dName').value.trim(),category=modal.querySelector('#dCategory').value.trim(),image=modal.querySelector('#dImage').value.trim(),ingredients=ingredientDraft.map(i=>({name:(i.name||'').trim(),amount:(i.amount||'').trim(),unit:(i.unit||'').trim()})).filter(i=>i.name);if(!name){alert('Bitte einen Namen eingeben.');return;}try{let id=dish?.id;if(editing)await this.ws('meal_vote/update_dish',{dish_id:id,name,category,image,active:modal.querySelector('#dActive').checked,ingredients});else{const r=await this.ws('meal_vote/add_dish',{name,category,image,ingredients});id=r.dish_id;}const file=modal.querySelector('#dFile')?.files?.[0]||null;if(file)await this.uploadImage(id,file);dialog.close();await this.load();}catch(e){alert(e.message||e);}};
...
if(!customElements.get('meal-vote-card')) customElements.define('meal-vote-card',MealVoteCard);console.info('[meal_vote] UI 0.4.16 loaded');window.customCards=window.customCards||[];window.customCards.push({type:'meal-vote-card',name:'Essenswahl',description:'Familien-Voting für Gerichte'});