class MealVoteCard extends HTMLElement {
  ...
  render(){
    ...
    this.shadowRoot.innerHTML=`<style>
      ...
      <div class="top"><div class="searchWrap"><input class="search" id="search" placeholder="🔎 Gericht, Kategorie, Person oder Zutat suchen…" value="${this.esc(this._search)}"><button type="button" class="searchClear" id="searchClear" title="Suche löschen" aria-label="Suche löschen" style="${this._search?'':'display:none'}">✕</button></div><select id="sort"><option value="votes">Meiste Stimmen</option><option value="oldest">Lange nicht gekocht</option><option value="recent">Zuletzt gekocht</option><option value="name">Name</option></select><span style="font-size:.8rem;opacity:.7;font-weight:700">UI 0.6.16</span><button id="reload">↻ Sync</button><button id="inactive">${this._showInactive?'Aktive':'Verwaltung'}</button>${this._showInactive?`<button id="pantry">🏠 Standardvorrat</button><button id="optimizeImages">🖼 Bilder optimieren</button><button id="importRecipe">📥 Rezept importieren</button><button id="add" class="add">＋ Gericht</button>`:''}</div>
...
if(!customElements.get('meal-vote-card')) customElements.define('meal-vote-card',MealVoteCard);console.info('[meal_vote] UI 0.6.16 loaded');window.customCards=window.customCards||[];window.customCards.push({type:'meal-vote-card',name:'Essenswahl',description:'Familien-Voting für Gerichte'});